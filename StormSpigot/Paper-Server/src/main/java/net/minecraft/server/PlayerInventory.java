...
public class PlayerInventory implements IInventory, INamableTileEntity {
	...
	private boolean isSimilarAndNotFull(ItemStack itemstack, ItemStack itemstack1) {
		return !itemstack.isEmpty() && this.b(itemstack, itemstack1) && itemstack.isStackable()
				&& itemstack.getCount() < itemstack.getMaxStackSize() && itemstack.getCount() < this.getMaxStackSize();
	}
	...
	public int canHold(ItemStack itemstack) {
		...
		for (int i = 0; i < this.items.size(); ++i) {
			...
			if (this.isSimilarAndNotFull(itemstack1, itemstack)) {
				remains -= (itemstack1.getMaxStackSize() < this.getMaxStackSize() ? itemstack1.getMaxStackSize()
						: this.getMaxStackSize()) - itemstack1.getCount();
			}
			if (remains <= 0)
				return itemstack.getCount();
		}
		...
		if (this.isSimilarAndNotFull(offhandItemStack, itemstack)) {
			remains -= (offhandItemStack.getMaxStackSize() < this.getMaxStackSize() ? offhandItemStack.getMaxStackSize()
	...
	public int c(ItemStack itemstack) {
		for (int i = 0; i < this.items.size(); ++i) {
			...
			if (!((ItemStack) this.items.get(i)).isEmpty() && this.b(itemstack, (ItemStack) this.items.get(i))
					&& !((ItemStack) this.items.get(i)).f() && !itemstack1.hasEnchantments() && !itemstack1.hasName()) {
				return i;
			}
		}
	...
	public boolean c(int i, ItemStack itemstack) {
		if (itemstack.isEmpty()) {
			...
			try {
				...
				crashreportsystemdetails.a("Item name", () -> { return itemstack.getName().getString(); });
			...
			while (!itemstack.isEmpty()) {
				...
				if (this.c(i, itemstack.cloneAndSubtract(j))) {
					((EntityPlayer) this.player).playerConnection
							.sendPacket(new PacketPlayOutSetSlot(-2, i, this.getItem(i)));
				}
			}
			...
			for (int i = 0; i < this.armor.size(); ++i) {
				...
				if (itemstack.getItem() instanceof ItemArmor) {
					...
					itemstack.damage((int) f, this.player, (entityhuman) -> {
						entityhuman.broadcastItemBreak(EnumItemSlot.a(EnumItemSlot.Function.ARMOR, finalI)); // CraftBukkit
																												// -
																												// decompile
																												// error
					});
