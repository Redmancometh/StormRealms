...
public abstract class MinecraftServer extends IAsyncTaskHandlerReentrant<TickTask>
		implements IMojangStatistics, ICommandListener, AutoCloseable, Runnable {
	...
	public static final WorldSettings c = (new WorldSettings((long) "North Carolina".hashCode(), EnumGamemode.SURVIVAL,
			true, false, WorldType.NORMAL)).a();
	public Convertable convertable;
	private final MojangStatisticsGenerator snooper = new MojangStatisticsGenerator("server", this,
			SystemUtils.getMonotonicMillis());
	...
	private long Z;
	public final Thread serverThread = (Thread) SystemUtils.a((new Thread(this, "Server thread")), (thread) -> { // CraftBukkit
																													// -
																													// decompile
																													// error
	...
	public MinecraftServer(OptionSet options, Proxy proxy, DataFixer datafixer, CommandDispatcher commanddispatcher,
			...
			UserCache usercache, WorldLoadListenerFactory worldloadlistenerfactory, String s) {
		super("Server");
		...
		// this.convertable = new Convertable(file.toPath(),
		// file.toPath().resolve("../backups"), datafixer); // CraftBukkit - moved to
	...
	private void initializeScoreboards(WorldPersistentData worldpersistentdata) {
		PersistentScoreboard persistentscoreboard = (PersistentScoreboard) worldpersistentdata
				.a(PersistentScoreboard::new, "scoreboard");
			...
			if (worlddata != null) {
				...
				while (!worldupgrader.b()) {
					...
					if (i > 0) {
						...
						MinecraftServer.LOGGER.info("{}% completed ({} / {} chunks)...",
	...
	protected void a(String s, String s1, long i, WorldType worldtype, JsonElement jsonelement) {
		...
		/*
		 * CraftBukkit start - Remove ticktime arrays and worldsettings WorldNBTStorage
		 * worldnbtstorage = this.getConvertable().a(s, this);
		 * 
		 * this.a(this.getWorld(), worldnbtstorage); WorldData worlddata =
		 * worldnbtstorage.getWorldData(); WorldSettings worldsettings;
		 * 
		 * if (worlddata == null) { if (this.isDemoMode()) { worldsettings =
		 * MinecraftServer.c; } else { worldsettings = new WorldSettings(i,
		 * this.getGamemode(), this.getGenerateStructures(), this.isHardcore(),
		 * worldtype); worldsettings.setGeneratorSettings(jsonelement); if
		 * (this.bonusChest) { worldsettings.a(); } }
		 * 
		 * worlddata = new WorldData(worldsettings, s1); } else { worlddata.setName(s1);
		...
		for (int j = 0; j < worldCount; ++j) {
			...
			WorldSettings worldsettings = new WorldSettings(
					com.destroystokyo.paper.PaperConfig.seedOverride.getOrDefault(name, i), this.getGamemode(),
					this.getGenerateStructures(), this.isHardcore(), worldtype); // Paper
			worldsettings.setGeneratorSettings(jsonelement);

			if (j == 0) {
				WorldNBTStorage worldnbtstorage = new WorldNBTStorage(server.getWorldContainer(), s1, this,
				...
				world = new WorldServer(this, this.executorService, worldnbtstorage, worlddata,
						DimensionManager.OVERWORLD, this.methodProfiler, worldloadlistener,
						org.bukkit.World.Environment.getEnvironment(dimension), gen);
				...
				File oldLevelDat = new File(new File(s), "level.dat"); // The data folders exist on first run as they
																		// are created in the PersistentCollection
																		// constructor above, but the level.dat won't
				...
				WorldNBTStorage worldnbtstorage = new WorldNBTStorage(server.getWorldContainer(), name, this,
						this.dataConverterManager);
				...
				worlddata.checkName(name); // CraftBukkit - Migration did not rewrite the level.dat; This forces 1.8 to
											// take the last loaded world as respawn (in this case the end)
				WorldLoadListener worldloadlistener = this.worldLoadListenerFactory.create(11);
				world = new SecondaryWorldServer(this.getWorldServer(DimensionManager.OVERWORLD), this,
						this.executorService, worldnbtstorage, DimensionManager.a(dimension), this.methodProfiler,
						worldloadlistener, worlddata, org.bukkit.World.Environment.getEnvironment(dimension), gen);
			}
		...
		for (WorldServer worldserver : com.google.common.collect.Lists.newArrayList(this.getWorlds())) { // Paper -
																											// avoid
																											// como if 1
																											// world
																											// triggers
		...
		if (!com.destroystokyo.paper.PaperConfig.enablePlayerCollisions) {
			this.getPlayerList().collideRuleTeamName = org.apache.commons.lang3.StringUtils
					.left("collideRule_" + java.util.concurrent.ThreadLocalRandom.current().nextInt(), 16);
	...
	public void initWorld(WorldServer worldserver1, WorldData worlddata, WorldSettings worldsettings) {
		...
		if (worldserver1.generator != null) {
			worldserver1.getWorld().getPopulators()
					.addAll(worldserver1.generator.getDefaultPopulators(worldserver1.getWorld()));
		}
		// CraftBukkit end
	...
	public void loadSpawn(WorldLoadListener worldloadlistener, WorldServer worldserver) {
		...
		MinecraftServer.LOGGER.info("Preparing start region for dimension '{}'/{}",
				worldserver.getWorldData().getName(),
				DimensionManager.a(worldserver.worldProvider.getDimensionManager().getType())); // CraftBukkit
		...
		// Paper end
		LOGGER.info("Loaded " + chunkproviderserver.b() + " spawn chunks for world "
				+ worldserver.getWorldData().getName()); // Paper
		...
		// CraftBukkit end
		worldloadlistener.b();
		chunkproviderserver.getLightEngine().a(worldserver.paperConfig.lightQueueSize); // Paper - increase light queue
																						// size
	...
	public boolean saveChunks(boolean flag, boolean flag1, boolean flag2) {
		...
		for (Iterator iterator = this.getWorlds().iterator(); iterator.hasNext(); flag3 = true) {
			...
			if (!flag) {
				MinecraftServer.LOGGER.info("Saving chunks for level '{}'/{}", worldserver.getWorldData().getName(),
						DimensionManager.a(worldserver.worldProvider.getDimensionManager()));
			}
		 ...
		 * worldserver1.getDataManager().saveWorldData(worlddata,
		 * this.getPlayerList().save());
		 */
		// CraftBukkit end
		return flag3;
	}
	...
	protected void stop() {
		...
		// CraftBukkit end
		MinecraftServer.LOGGER.info(
				"Stopping server (Ignore any thread death message you see! - DO NOT REPORT THREAD DEATH TO PAPER)"); // Paper
	...
	public void run() {
		try {
			...
			if (this.init()) {
				...
				this.serverPing.setServerInfo(new ServerPing.ServerData(SharedConstants.getGameVersion().getName(),
				...
				// this is going to be the first thing the tick process does anyways, so move
				// done and run it after
				// everything is init before watchdog tick.
				// anything at 3+ won't be caught here but also will trip watchdog....
				// tasks are default scheduled at -1 + delay, and first tick will tick at 1
				String doneTime = String.format(java.util.Locale.ROOT, "%.3fs",
						(double) (SystemUtils.getMonotonicNanos() - serverStartTime) / 1.0E9D);
				...
				while (this.isRunning) {
					...
					// CraftBukkit // Paper - don't overwrite current tick time
					lastTick = curTime;
					this.nextTick += 50L;
					if (this.T) {
						this.T = false;
			...
			File file = new File(new File(this.z(), "crash-reports"),
			...
			try {
				...
				try {
					// net.minecrell.terminalconsole.TerminalConsoleAppender.close(); // Paper -
					// Move into stop
	...
	public boolean canSleepForTick() { // Paper
		...
		return this.forceTicks || this.isEntered()
				|| SystemUtils.getMonotonicMillis() < (this.ac ? this.ab : this.nextTick);
	}
	...
	protected void a(BooleanSupplier booleansupplier) {
		...
		if (i - this.Z >= 5000000000L) {
			this.Z = i;
			this.serverPing.setPlayerSample(
					new ServerPing.ServerPingPlayerSample(this.getMaxPlayers(), this.getPlayerCount()));
			GameProfile[] agameprofile = new GameProfile[Math.min(this.getPlayerCount(),
					org.spigotmc.SpigotConfig.playerSample)]; // Paper
		...
		if (((DedicatedServer) this).getDedicatedServerProperties().snooperEnabled && !this.snooper.d()
				&& this.ticks > 100) { // Spigot
			this.snooper.a();
		}
		...
		new com.destroystokyo.paper.event.server.ServerTickEndEvent(this.ticks,
				((double) (endTime - lastTick) / 1000000D), remaining).callEvent();
		// Paper end
		...
		while (iterator.hasNext()) {
			...
			TileEntityHopper.skipHopperEvents = worldserver.paperConfig.disableHopperMoveEvents
					|| org.bukkit.event.inventory.InventoryMoveItemEvent.getHandlerList()
							.getRegisteredListeners().length == 0; // Paper
			if (true || worldserver.worldProvider.getDimensionManager() == DimensionManager.OVERWORLD
					|| this.getAllowNether()) { // CraftBukkit
				this.methodProfiler.a(() -> {
					return worldserver.getWorldData().getName() + " "
							+ IRegistry.DIMENSION_TYPE.getKey(worldserver.worldProvider.getDimensionManager());
				});
				/*
				 * Drop global time updates if (this.ticks % 20 == 0) {
				 * this.methodProfiler.enter("timeSync"); this.playerList.a((Packet) (new
				 * PacketPlayOutUpdateTime(worldserver.getTime(), worldserver.getDayTime(),
				 * worldserver.getGameRules().getBoolean(GameRules.DO_DAYLIGHT_CYCLE))),
				 * worldserver.worldProvider.getDimensionManager()); this.methodProfiler.exit();
				 * } // CraftBukkit end
				 */
	...
	public static void main(final OptionSet optionset) { // CraftBukkit - replaces main(String[] astring)
		try {
			...
			if (optionset.has("initSettings")) { // CraftBukkit
				MinecraftServer.LOGGER.info("Initialized '" + java_nio_file_path.toAbsolutePath().toString() + "' and '"
						+ java_nio_file_path1.toAbsolutePath().toString() + "'");
				return;
			}
			// Spigot Start
			CrashReport.h();
			...
			// CraftBukkit start
			String s1 = (String) Optional.ofNullable(optionset.valueOf("world"))
					.orElse(dedicatedserversettings.getProperties().levelName);
			final DedicatedServer dedicatedserver = new DedicatedServer(optionset, dedicatedserversettings,
					DataConverterRegistry.a(), yggdrasilauthenticationservice, minecraftsessionservice,
					gameprofilerepository, usercache, WorldLoadListenerLogger::new, s1);
	...
	public void startServerThread() {
		/*
		 * CraftBukkit start - prevent abuse this.serverThread.start(); // CraftBukkit
		 * end
		 */
	}
	...
	public CrashReport b(CrashReport crashreport) {
		if (this.playerList != null) {
			crashreport.g().a("Player Count", () -> {
				return this.playerList.getPlayerCount() + " / " + this.playerList.getMaxPlayers() + "; "
	...
	public void sendMessage(IChatBaseComponent ichatbasecomponent) {
		MinecraftServer.LOGGER.info(org.bukkit.craftbukkit.util.CraftChatMessage.fromComponent(ichatbasecomponent));// Paper
																													// -
																													// Log
																													// message
																													// with
	...
	public void a(MojangStatisticsGenerator mojangstatisticsgenerator) {
		...
		if (this.playerList != null) {
			...
			mojangstatisticsgenerator.a("players_seen",
					this.getWorldServer(DimensionManager.OVERWORLD).getDataManager().getSeenPlayers().length);
		}

		mojangstatisticsgenerator.a("uses_auth", this.onlineMode);
		mojangstatisticsgenerator.a("gui_state", this.aj() ? "enabled" : "disabled");
		mojangstatisticsgenerator.a("run_time",
				(SystemUtils.getMonotonicMillis() - mojangstatisticsgenerator.g()) / 60L * 1000L);
	...
	public ServerConnection getServerConnection() {
		return this.serverConnection == null ? this.serverConnection = new ServerConnection(this)
				: this.serverConnection; // Spigot
	}
				...
				while (iterator.hasNext()) {
					...
					if (!whitelist.isWhitelisted(entityplayer.getProfile())) {
						entityplayer.playerConnection
								.disconnect(new ChatMessage("multiplayer.disconnect.not_whitelisted", new Object[0]));
					}
				}
	...
	public CommandListenerWrapper getServerCommandListener() {
		return new CommandListenerWrapper(this,
				this.getWorldServer(DimensionManager.OVERWORLD) == null ? Vec3D.a
						: new Vec3D(this.getWorldServer(DimensionManager.OVERWORLD).getSpawn()),
				Vec2F.a, this.getWorldServer(DimensionManager.OVERWORLD), 4, "Server", new ChatComponentText("Server"),
				this, (Entity) null);
	}
	...
	public int b(GameProfile gameprofile) {
		if (this.getPlayerList().isOp(gameprofile)) {
			...
			return oplistentry != null ? oplistentry.a()
					: (this.a(gameprofile) ? 4
							: (this.isEmbeddedServer() ? (this.getPlayerList().v() ? 4 : 0) : this.j()));
		} else {
			return 0;
		}
	}
	...
	public void a(java.nio.file.Path java_nio_file_path) throws IOException {
		...
		while (iterator.hasNext()) {
			...
			java.nio.file.Path java_nio_file_path2 = java_nio_file_path1.resolve(minecraftkey.getNamespace())
					.resolve(minecraftkey.getKey());
		...
		try {
			...
			GameRules.a(new GameRules.GameRuleVisitor() {
				...
				public <T extends GameRules.GameRuleValue<T>> void a(GameRules.GameRuleKey<T> gamerules_gamerulekey,
					...
					list.add(String.format("%s=%s\n", gamerules_gamerulekey.a(),
							gamerules.get(gamerules_gamerulekey).toString()));
				}
			});
			Iterator iterator = list.iterator();
	...
	public boolean isMainThread() {
		return super.isMainThread() /* || this.isStopped() */; // CraftBukkit - MC-142590 // Paper - causes issues
																// elsewhere
	}
