...
public abstract class IRegistry<T> implements Registry<T> {
	...
	public static final IRegistry<WorldGenFeatureDefinedStructurePools> STRUCTURE_POOL_ELEMENT = a(
	...
	public static final RegistryBlocks<VillagePlaceType> POINT_OF_INTEREST_TYPE = a("point_of_interest_type",
			"unemployed", () -> {
	...
	private static <T, R extends IRegistryWritable<T>> R a(String s, R r0, Supplier<T> supplier) {
		...
		return IRegistry.f.a(minecraftkey, r0);
	}
	...
	public static <T> T a(IRegistry<? super T> iregistry, MinecraftKey minecraftkey, T t0) {
		return (T) ((IRegistryWritable) iregistry).a(minecraftkey, t0);
	}

	public static <T> T a(IRegistry<? super T> iregistry, int i, String s, T t0) {
		return (T) ((IRegistryWritable) iregistry).a(i, new MinecraftKey(s), t0);
	}
	...
	static {
		...
		IRegistry.f.forEach((iregistrywritable) -> {
			if (iregistrywritable.c()) {
				...
				if (SharedConstants.b) {
					throw new IllegalStateException(
							"Registry: '" + IRegistry.f.getKey(iregistrywritable) + "' is empty, not allowed, fix me!");
				}
			}
			...
			if (iregistrywritable instanceof RegistryBlocks) {
				...
				Validate.notNull(iregistrywritable.get(minecraftkey),
						"Missing default of DefaultedMappedRegistry: " + minecraftkey, new Object[0]);
			}
