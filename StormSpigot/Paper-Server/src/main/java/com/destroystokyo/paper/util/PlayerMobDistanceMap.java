...
public final class PlayerMobDistanceMap {
	...
	private final Long2ObjectOpenHashMap<PooledHashSets.PooledObjectLinkedOpenHashSet<EntityPlayer>> playerMap = new Long2ObjectOpenHashMap<>(
			32, 0.5f);
	private int viewDistance;

	private final PooledHashSets<EntityPlayer> pooledHashSets = new PooledHashSets<>();

	public PooledHashSets.PooledObjectLinkedOpenHashSet<EntityPlayer> getPlayersInRange(
			final ChunkCoordIntPair chunkPos) {
		return this.getPlayersInRange(chunkPos.x, chunkPos.z);
	}

	public PooledHashSets.PooledObjectLinkedOpenHashSet<EntityPlayer> getPlayersInRange(final int chunkX,
			final int chunkZ) {
	...
	private void validatePlayer(final EntityPlayer player, final int viewDistance) {
		...
		for (final Entry<PooledHashSets.PooledObjectLinkedOpenHashSet<EntityPlayer>> entry : this.playerMap
				.long2ObjectEntrySet()) {
	...
	private void addPlayerTo(final EntityPlayer player, final int chunkX, final int chunkZ) {
		this.playerMap.compute(ChunkCoordIntPair.pair(chunkX, chunkZ),
				(final Long key, final PooledHashSets.PooledObjectLinkedOpenHashSet<EntityPlayer> players) -> {
					if (players == null) {
	...
	private void removePlayerFrom(final EntityPlayer player, final int chunkX, final int chunkZ) {
		this.playerMap.compute(ChunkCoordIntPair.pair(chunkX, chunkZ),
				(final Long keyInMap, final PooledHashSets.PooledObjectLinkedOpenHashSet<EntityPlayer> players) -> {
					return PlayerMobDistanceMap.this.pooledHashSets.findMapWithout(players, player); // rets null
																										// instead of an
																										// empty map
				});
	}

	private void updatePlayer(final EntityPlayer player, final SectionPosition oldPosition,
			final SectionPosition newPosition, final int oldViewDistance, final int newViewDistance) {
