...
public class PacketSubscriptionManager implements PacketDispatcher {
	...
	private Map<Class<? extends Packet<PacketListenerPlayIn>>, List<PacketSubscription<PacketHook<? extends Packet<PacketListenerPlayIn>>>>> subscriptions = new HashMap<>();

	/**
	 * Creates a subscription to a specific packet inheriting
	 * {@link Packet}{@literal <PacketListenerPlayIn>}.
	 * 
	 * @param <T>         The type of packet to subscribe to.
	 * @param packetClass The {@link Class} that represents the type of packet to
	 *                    subscribe to.
	...
	public <T extends Packet<PacketListenerPlayIn>> PacketSubscription<PacketHook<T>> subscribe(Class<T> packetClass) {
		...
		List<PacketSubscription<PacketHook<? extends Packet<PacketListenerPlayIn>>>> packetSubList = subscriptions
				.get(packetClass);
		...
		packetSubList
				.add((PacketSubscription<PacketHook<? extends Packet<PacketListenerPlayIn>>>) (Object) subscription);
		return subscription;
	}
	 ...
	 * @param dispatchInternal A function that calls to the internal packet
	 *                         listener.
